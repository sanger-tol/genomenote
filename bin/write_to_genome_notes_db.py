...
def fetch_ads(url, key):
    ads = ApiDataSource({"url": url, "key": key})
    return ads
...
def write_to_db(param_file, url, key):
    ...
    if species_name:
        species_exists = ads.get_list("species", DataSourceFilter(exact={"scientific_name": species_name}))

        if len(list(species_exists)) != 0:
            species = species_exists[0]
        else:
            species = ApiObject("species", None, attributes={"scientific_name": params.get("GENUS_SPECIES")})
    ...
    template_exists = ads.get_list(
        "template", DataSourceFilter(exact={"name": "WOR_Standard", "journal": "Wellcome Open Research"})
    )
    ...
    if len(list(template_exists)) != 0:
        ...
        template = ApiObject(
            "template",
            None,
            attributes={"name": "WOR_Standard", "journal": "Wellcome Open Research", "template_body": ""},
        )
    ...
    if assembly_accession:
        ...
        if len(list(assembly_exists)) != 0:
            ...
            assembly = ApiObject(
                "assembly",
                None,
                attributes={"accession": assembly_accession, "name": assembly_name, "taxon_id": taxon_id},
            )
        ...
        for parameter in params:
            ...
            parameter_class_exists = ads.get_list(
                "parameter_classes", DataSourceFilter(exact={"name": parameter, "jats": parameter})
            )

            if len(list(parameter_class_exists)) != 0:
                parameter_class = parameter_class_exists[0]
            else:
                parameter_class = ApiObject(
                    "parameter_classes", None, attributes={"name": parameter, "jats": parameter}
                )

                parameter_class = ads.create(parameter_class)

            parameter_exists = ads.get_list(
                "parameters",
                DataSourceFilter(
                    exact={
            ...
            if len(list(parameter_exists)) != 0:
                ...
                parameters = ApiObject(
                    "parameters",
                    None,
                    attributes={"value": parameter_value, "assembly_accession": assembly_accession},
                    relationships={"parameter_classes": parameter_class},
                )

                parameters = ads.create(parameters)

            template_parameter_exists = ads.get_list(
                "template_parameters",
                DataSourceFilter(
                    exact={
            ...
            if len(list(template_parameter_exists)) != 0:
                ...
                template_parameter = ApiObject(
                    "template_parameters",
                    None,
                    attributes={"required": True},
                    relationships={"template": template, "parameters": parameters},
                )
