process CREATETABLE {
    ...
    script:
    // This script is bundled with the pipeline, in sanger-tol/genomenote/bin/
    ...
    def mqv = [qv].flatten().collect { path -> "--qv ${path}" }.join(' ')
    def mco = [completeness].flatten().collect {  path -> "--completeness " + path }.join(' ')
    def hic = meta3s.collect { flagstat_meta -> "--hic " + flagstat_meta.id }.join(' ')
    def fst = [flagstats].flatten().collect { path -> "--flagstat " + path }.join(' ')
    """
    create_table.py \\
        ${gen} \\
        ${seq} \\
